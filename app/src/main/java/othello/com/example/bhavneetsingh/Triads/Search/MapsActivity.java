...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback ,NavigationView.OnNavigationItemSelectedListener, ProfileAdapter.FollowListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        profileAdapter=new ProfileAdapter(this,users,this,new User(current_user_id,"",""),followers);
    ...
    */    mapFragment.getMapAsync(this);
    ...
    public void refresh()
    {
        ...
        DBManager.searchUser(search_key,new OnDownloadComplete<ArrayList<User>>() {
            @Override
            public void onDownloadComplete(ArrayList<User> answer) {
                loading=false;
                if(answer!=null)
                {

                 final ArrayList<User>result=answer;
                 DBManager.fetchFollowers(current_user_id, new OnDownloadComplete<ArrayList<User>>() {
                     @Override
                     public void onDownloadComplete(ArrayList<User> result1) {
                         if(result1!=null)
                         {
                             followers.clear();
                             locations.clear();
